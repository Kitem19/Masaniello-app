...
const NumericCellInput: React.FC<NumericCellInputProps> = ({ value, onSave, disabled, className }) => {
    ...
    return (
        <input
            ...
            onBlur={handleSave}
            onKeyDown={(e) => { if (e.key === 'Enter') { handleSave(); (e.target as HTMLInputElement).blur(); } }}
            disabled={disabled}
            className={className}
        />
    );
                            ...
                            return (
                                <tr key={row.id} className="hover:bg-slate-700/50 transition-colors duration-150">
                                    ...
                                    <td className="p-1">
                                        <NumericCellInput
                                            value={row.odds}
                                            onSave={(newValue) => onUpdateRow(row.id, { odds: newValue })}
                                            disabled={!editable}
                                    ...
                                    <td className="p-1">
                                         <NumericCellInput
                                            value={row.stake}
                                            onSave={(newValue) => onUpdateRow(row.id, { stake: newValue })}
                                    ...
                                    <td className="p-1">
                                         <NumericCellInput
                                            value={row.liability}
                                            onSave={(newValue) => onUpdateRow(row.id, { liability: newValue })}
